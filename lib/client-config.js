...
// docs/tov-profile-bottbott.md, loaded by lib/generation/prompts.js).
//
...
export const CLIENT = {
  ...
  timezone: 'America/Chicago',                 // IANA tz — drives publish-date logic

  // --- JSON-LD schema --------------------------------------------------------
  // schema.org types for the business, most specific first,
  // e.g. ['RealEstateAgent', 'LocalBusiness'] — or just ['LocalBusiness'].
  // ProfessionalService — matches the homepage's canonical org node so Google
  // consolidates every page's org entity under the same @id + type.
  schemaTypes: ['ProfessionalService'],
  schemaImageUrl: 'https://www.bottbottgenai.com/BottBott.png', // logo image for JSON-LD "image"

  // --- pricing facts (injected into the generation prompts) ----------------
  // The single source of truth for any price the model may state. A published
  // post previously carried a stale price recalled from an old transcript.
  autoblogPrice: '$1,800',
  ...
  // Must stay in sync with the CHECK constraints (supabase/migrations/0001) and
  // the <option>s in admin/index.html.
  topicCategories: [
    'ai-roi-efficiency',
  ...
  topicAgentContext: 'You are helping Simon D, founder of BottBott Business Systems, an AI systems builder who designs automation systems — content pipelines, outreach systems, and custom web apps — that small businesses own outright. He writes about practical AI adoption, workflow automation, and owning your tools instead of renting platforms. His voice is plain, direct, and first-person — no hype, no fluff.',
  ...
  // Pool A = the client's primary site pages, as a dash-list string.
  // Pool B = approved off-site links treated as internal, each an object
  // with a use-when condition; rendered into the prompt by prompts.js.
  internalLinkPoolA: `- Home: https://www.bottbottgenai.com/
- Systems: https://www.bottbottgenai.com/systems/
...
- About: https://www.bottbottgenai.com/about/`,
  internalLinkPoolB: [
    {
      url: 'https://www.youtube.com/watch?v=EPqU1y5keOU',
      label: 'AI DIY podcast appearance',
      useWhen: 'Only when Simo references appearing on a podcast, being interviewed, or speaking publicly about his work.'
    },
    {
      url: 'https://bottbott.substack.com/',
      label: 'BottBott on Substack',
      useWhen: 'Only when Simo references his other writing on AI, systems, or building. Not as a general "read more" link.'
    },
    {
      url: 'https://www.lilidauphinee.com/',
      label: "Lili Dauphinee's art business",
      useWhen: "Only when Simo references his wife, her art business, or work he does on her operations. Never as a generic example."
    }
  ],
  ...
  blogHeroSubhead: 'Practical guidance: No hype. No fluff. Just what works for your business.',
  ...
  // --- post CTA block ------------------------------------------------------
  // REVIEW(Simo): reuses the homepage CTA-banner + contact copy. Primary
  // action books the Calendly slot; /strategy-call/ stays as the secondary
  // send-a-note option in templates/post.html.
  calendlyUrl: 'https://calendly.com/simon-bottbottgenai/30min',
  postCtaHeading: 'Ready to have systems that actually work for you?',
...
// Strip a www. prefix: Resend verifies the bare domain, and sends from an
// unverified subdomain (noreply@www.…) are rejected.
export const FROM_EMAIL = `${CLIENT.editorName} <noreply@${CLIENT.siteDomain.replace(/^www\./, '')}>`;
