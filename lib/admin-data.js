...
const mockState = {
  ...
  images: [],
  // Neutral examples: real production entries too, so the mock fixture and
  // the live default aren't telling two different stories.
  sources: [
    { domain: 'openai.com', trust_score: 90, risk_tier: 'trusted', first_seen: '2026-08-13T04:55:41Z' },
    { domain: 'anthropic.com', trust_score: 90, risk_tier: 'trusted', first_seen: '2026-08-13T04:55:41Z' },
  ],
};
...
// ------------------------------------------------ link-sweep trusted sources
// Domains here bypass the link sweep's freshness gate (lib/links/freshness.js)
// — for a source that's clearly authoritative but whose pages don't carry a
// scrapeable publish date (a company's own docs/blog, e.g. developers.google.com:
// no <meta> date tags, no dated JSON-LD, real content buried past the nav
// sidebar). Relevancy and trust are unaffected — this only waives "how old is
// this page," never "is this a good source."
//
// `domains` is the link sweep's own domain cache (supabase/schema.sql), so
// adding a source here reuses whatever's already cached for it (trust_score,
// risk_tier bookkeeping) rather than creating a second table to keep in sync.

export async function getAllowlistedSources() {
  if (MOCK()) return mockState.sources;
  const { data, error } = await getSupabaseClient()
    .from('domains').select('domain, trust_score, risk_tier, first_seen')
    .eq('allowlisted', true)
    .order('domain', { ascending: true });
  if (error) throw new Error(`domains read failed: ${error.message}`);
  return data;
}

// Upsert only touches the columns listed here — a domain the sweep has
// already scored (trust_score, trust_signals, business_type, ...) keeps that
// data; this never resets it. risk_tier is set alongside allowlisted per the
// invariant riskTierFor() computes at read time (lib/links/domains.js).
export async function addAllowlistedSource(domain) {
  if (MOCK()) {
    const existing = mockState.sources.find((s) => s.domain === domain);
    if (existing) return existing;
    const row = { domain, trust_score: null, risk_tier: 'trusted', first_seen: new Date().toISOString() };
    mockState.sources.push(row);
    return row;
  }
  const { data, error } = await getSupabaseClient()
    .from('domains')
    .upsert({ domain, allowlisted: true, risk_tier: 'trusted' }, { onConflict: 'domain' })
    .select('domain, trust_score, risk_tier, first_seen')
    .single();
  if (error) throw new Error(`domains upsert failed: ${error.message}`);
  return data;
}

// Un-allowlist rather than delete: the row is the sweep's cache entry for
// this domain, not just an allowlist marker, and other cached data on it
// (trust_score, cross_mention_count, ...) stays useful even if the freshness
// bypass is turned back off.
export async function removeAllowlistedSource(domain) {
  if (MOCK()) {
    mockState.sources = mockState.sources.filter((s) => s.domain !== domain);
    return;
  }
  const { error } = await getSupabaseClient()
    .from('domains')
    .update({ allowlisted: false, risk_tier: 'standard' })
    .eq('domain', domain);
  if (error) throw new Error(`domains update failed: ${error.message}`);
}
