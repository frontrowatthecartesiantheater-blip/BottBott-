...
import { SITE_ORIGIN } from './client-config.js';

export function buildPublishFiles({ pkg, date, manifest, imageFilename = null, imageAlt = null }) {
  const { post } = pkg;

  // Hard fail (non-negotiable): exactly one H1 or no publish.
  ...
  // The feed is a nice-to-have alongside the post itself: a bug in
  // renderFeed() must never block the publish. Fall back to committing
  // without feed.xml rather than aborting the whole commit.
  try {
    files.push({ path: 'feed.xml', content: renderFeed(posts) });
  } catch (err) {
    console.error(`renderFeed failed, publishing without feed.xml: ${err.message}`);
  }
...
async function writeSheetRow(row) {
  ...
  // Columns F-G are the client's manual checkboxes; their existing data validation
